...
namespace MusicApi.Controllers
{
    ...
    public class AlbumsController : ControllerBase
    {
        ...
        public async Task<IActionResult> Post([FromForm] Album album)
        {
            ...
            await _dbContext.AlbumList.AddAsync(album);
        ...
        public async Task<IActionResult> GetAlbums(int? pageNumber, int? pageSize)
        {
            ...
            var albums = await (from album in _dbContext.AlbumList
                                 select new
                                 {
                                     AlbumId = album.Id,
        ...
        {
            var albumDetials = await _dbContext.AlbumList.Where(a => a.Id == albumId).Include(a => a.Songs).ToListAsync();

            return Ok(albumDetials);
        }
    }
}
